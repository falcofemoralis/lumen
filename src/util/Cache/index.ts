/* eslint-disable @typescript-eslint/no-explicit-any -- Cache value can be any value */
/* eslint-disable @typescript-eslint/no-unsafe-return -- Cache value can be any value */
import { Cache } from 'react-native-cache';
import { cacheStorage } from 'Util/Storage';

type KeyValuePair = [string, string | null];
type OnMultiResult = (result: any[]) => any;
type OnResult = (error: unknown, result?: any) => void;
...
const backend = {
   ...
   * Fetches `key` value.
   */
  getItem: (key: string, callback = () => {}) => createPromise(
   ...
   * Sets `value` for `key`.
   */
  setItem: (key: string, value: string, callback = () => {}) => createPromise(
   ...
   * Removes a `key`
   */
  removeItem: (key: string, callback = () => {}) => createPromise(
   ...
   */
  getAllKeys: (callback = () => {}) => createPromise(() => [], callback),
   ...
   */
  multiRemove: (keys: string[], callback = () => {}) => createPromise(
    () => cacheStorage.clearStore(),
    callback,
  ),
};
