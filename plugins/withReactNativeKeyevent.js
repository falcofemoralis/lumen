...
const withAndroidMainActivityBody = function (config) {
  ...
  return (0, config_plugins_1.withMainActivity)(config, function (mainActivityConfig) {
    const newSrc = [
      ...
      '}',
      '// Keys the app navigates with. They are handed straight to KeyEventModule and',
      '// never passed to `super`, so Android\'s own focus engine never sees them: it',
      '// would otherwise move the native focus between the (always natively focusable)',
      '// RNGH buttons and auto-scroll their ScrollViews, fighting the spatial',
      '// navigation that actually drives the UI.',
      'private val consumedKeyCodes = setOf(',
